...
async function crawlAndCreate() {
	...
	await fs.writeFileSync('./result.txt', new Date() + ': r18s started' + JSON.stringify(before) + '\n', { flag : 'a'})
	...
	await fs.writeFileSync('./result.txt', new Date() + ': r18s ended'  + JSON.stringify(after) + '\n\n', { flag : 'a'})
}
